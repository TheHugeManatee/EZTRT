...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                                    \
    ...
    {                                                                                              \
        if (!(status))                                                                             \
        {                                                                                          \
            std::cerr << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME            \
        ...
        : mName(name)
    {
        for (const auto& srcProfiler : srcProfilers)
        {
            for (const auto& rec : srcProfiler.mProfile)
            {
                ...
                if (it == mProfile.end()) { mProfile.insert(rec); }
                else
                {
...
{
public:
    TypedHostMemory(std::size_t size) : HostMemory(size, dataType) { mData = new ElemType[size]; };
...
inline std::vector<std::string> classify(const std::vector<std::string>& refVector,
                                         const result_vector_t& output, const size_t topK)
{
...
inline void setDummyInt8Scales(const IBuilderConfig* c, INetworkDefinition* n)
{
    ...
    {
        spdlog::warn("TensorRT.common: Int8 calibrator not provided. Generating dummy per tensor "
                     "scales. Int8 "
                      ...
                      bool allowGPUFallback = true)
{
    if (useDLACore >= 0)
    {
        ...
        {
            spdlog::error(
                "Trying to use DLA core {} on a platform that doesn't have any DLA cores");
...
inline int parseDLA(int argc, char** argv)
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (strncmp(argv[i], "--useDLACore=", 13) == 0) return std::stoi(argv[i] + 13);
    }
    return -1;
}
...
inline void loadLibrary(const std::string& path)
{
    ...
    {
#ifdef _MSC_VER
        spdlog::error("TensorRT.common:  Could not load plugin library: {}", path);
#else
        spdlog::error("TensorRT.common: Could not load plugin library: {}, due to: {}", path,
                      dlerror());
#endif
    }
}
